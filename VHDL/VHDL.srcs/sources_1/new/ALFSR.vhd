...
BEGIN
    ...
    Feedback  <= CurrState(255) XOR CurrState(253) XOR CurrState(252) XOR CurrState(250); -- https://datacipy.cz/lfsr_table.pdf#page=3 (n = 255 @ LFSR-4)
    ...
    Valid <= '0' WHEN CurrState = (CurrState'RANGE => '0') OR CurrState = (CurrState'RANGE => '1') ELSE
        '1'; -- wait for the parallel load to kickstart the operation (accounting for the inverted seed inside the scrambler as well)
END behavioral;